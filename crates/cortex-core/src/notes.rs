...
pub struct Note {
    ...
    // Bi-temporal tracking (Graphiti-inspired)
    /// When this note's fact became true (None = since creation)
    #[serde(default)]
    pub valid_at: Option<DateTime<Utc>>,
    /// When this note's fact stopped being true (None = still valid)
    #[serde(default)]
...
pub struct ContextKnowledge {
    ...
    pub decisions: Vec<crate::models::DecisionNode>,
    /// Recent commits touching this entity (via TOUCHES)
    #[serde(skip_serializing_if = "Vec::is_empty", default)]
    pub recent_commits: Vec<crate::models::FileHistoryEntry>,
...
mod tests {
    ...
    ///
    /// In IndentiaGraph, the Cypher query computes:
