...
/// A code entity node extracted from the IndentiaGraph knowledge graph.
...
///
/// This is the intermediate representation between IndentiaGraph extraction and
...
// ============================================================================
// Writer types — Analytics updates for IndentiaGraph persistence
// ============================================================================

/// Batch update payload for File nodes' analytics properties.
///
/// Used by the `AnalyticsWriter` to persist computed scores back to IndentiaGraph
...
///
/// Retrieved from IndentiaGraph in a single query per project. The handler converts
...
/// that can be stored as IndentiaGraph node properties (cc_* prefix) and served
...
pub struct TopologyRule {
    ...
    /// Glob pattern for source files/functions (e.g. "src/indentiagraph/**")
...
/// Convert a glob pattern to a regex pattern suitable for IndentiaGraph.
///
/// Supports:
/// - `**` → `.*` (match any depth)
...
/// # Examples
/// ```
/// use cortex_core::graph::glob_to_regex;
/// assert_eq!(glob_to_regex("src/indentiagraph/**"), "^src/indentiagraph/.*$");
...
// ============================================================================
// Bridge Subgraph — raw IndentiaGraph results (Plan 1 GraIL)
// ============================================================================
...
mod tests {
    ...
    fn test_glob_to_regex_double_star() {
        assert_eq!(
            glob_to_regex("src/indentiagraph/**"),
            "^src/indentiagraph/.*$"
        );
    }
    ...
    fn test_topology_rule_serde_roundtrip() {
        let rule = TopologyRule {
            ...
            source_pattern: "src/indentiagraph/**".to_string(),
            target_pattern: Some("src/api/**".to_string()),
            threshold: None,
            severity: TopologySeverity::Error,
            description: "IndentiaGraph must not import API".to_string(),
        };
    ...
    fn test_topology_violation_serde() {
        let violation = TopologyViolation {
            ...
            rule_type: TopologyRuleType::MustNotImport,
            violator_path: "src/indentiagraph/client.rs".to_string(),
        ...
        assert_eq!(deserialized.violator_path, "src/indentiagraph/client.rs");
