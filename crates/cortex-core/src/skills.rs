...
use crate::neurons::ActivatedNote;
...
pub enum TriggerType {
    /// Regular expression pattern (e.g., `indentiagraph|cypher|UNWIND`)
    Regex,
    /// File glob pattern (e.g., `src/indentiagraph/**`)
    FileGlob,
...
/// (e.g., "IndentiaGraph Performance", "API Authentication") and can be automatically
/// activated via Claude Code hooks to inject relevant context.
///
/// # IndentiaGraph Relations
/// ```text
...
mod tests {
    ...
    fn test_skill_trigger_constructors() {
        let regex = SkillTrigger::regex("indentiagraph|cypher", 0.6);
        assert_eq!(regex.pattern_type, TriggerType::Regex);
        assert_eq!(regex.pattern_value, "indentiagraph|cypher");
        assert_eq!(regex.confidence_threshold, 0.6);
        assert!(regex.quality_score.is_none());

        let glob = SkillTrigger::file_glob("src/indentiagraph/**", 0.8);
        assert_eq!(glob.pattern_type, TriggerType::FileGlob);
        assert_eq!(glob.pattern_value, "src/indentiagraph/**");
    ...
    fn test_skill_trigger_serde_roundtrip() {
        let trigger = SkillTrigger::regex("indentiagraph|cypher", 0.6);
    ...
    fn test_skill_node_new() {
        ...
        let skill = SkillNode::new(project_id, "IndentiaGraph Performance");
    ...
    fn test_skill_node_serde_roundtrip() {
        ...
        skill.trigger_patterns = vec![
            SkillTrigger::regex("indentiagraph|cypher|UNWIND", 0.6),
            SkillTrigger::file_glob("src/indentiagraph/**", 0.8),
        ];
        ...
        assert_eq!(deserialized.name, "IndentiaGraph Performance");
    ...
    fn test_hook_activate_response_serde() {
        let resp = HookActivateResponse {
            context: "## IndentiaGraph Performance\nSome context".to_string(),
            skill_name: "IndentiaGraph Performance".to_string(),
        ...
        assert_eq!(deserialized.skill_name, "IndentiaGraph Performance");
