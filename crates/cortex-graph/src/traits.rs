...
//!
//! Defines the abstract interface for all IndentiaGraph graph operations.
//! This trait mirrors all public async methods of `IndentiaGraphClient`,
...
///
/// Every public async method of `IndentiaGraphClient` (excluding `new`, `init_schema`,
...
pub trait GraphStore: Send + Sync {
    ...
    /// Semantic search over Decision embeddings using IndentiaGraph vector index.
    ...
    /// Persists top-N link predictions as PREDICTED_LINK relationships in IndentiaGraph.
    ...
    ///
    /// Stores the rule as a `TopologyRule` node in IndentiaGraph with the given
    ...
    /// Read a single context card from IndentiaGraph cc_* properties.
    ...
    ///
    /// Returns notes with relevance scores, ordered by descending score.
    /// Active and needs_review notes only (excludes obsolete/archived).
    ...
    ///
    /// Returns decisions with relevance scores, ordered by descending score.
    ...
    ///
    /// Searches the `name` and `docstring` fields on both `function` and `struct` tables.
    /// Results are grouped by file path into `CodeSearchHit` entries.
    async fn search_code_fts(
        ...
        project_id: Option<&str>,
        language: Option<&str>,
    ) -> Result<Vec<crate::CodeSearchHit>>;

    // =========================================================================
    // Episodic Memory (Graphiti-inspired temporal knowledge capture)
    // =========================================================================
    ...
    /// Mark a note as invalid at a specific point in time (bi-temporal invalidation).
    /// Sets `invalid_at` to `at` and updates status to Obsolete.
    /// The note is preserved for historical queries.
    async fn invalidate_note_at(&self, id: &str, at: DateTime<Utc>) -> Result<()>;

    /// Search notes that were active at a specific point in time.
    /// Returns notes where `valid_at <= at AND (invalid_at IS NULL OR invalid_at > at)`.
