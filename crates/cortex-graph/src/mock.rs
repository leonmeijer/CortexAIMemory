...
pub struct MockGraphStore {
    ...
    pub refresh_tokens: RwLock<HashMap<String, cortex_core::models::RefreshTokenNode>>,
    ...
    pub file_analytics: RwLock<HashMap<String, cortex_core::graph::FileAnalyticsUpdate>>,
    /// Analytics scores stored for Function nodes (keyed by function id: "file_path:name:line_start")
    pub function_analytics: RwLock<HashMap<String, cortex_core::graph::FunctionAnalyticsUpdate>>,
    ...
    // Skill stores
    pub skills: RwLock<HashMap<Uuid, cortex_core::skills::SkillNode>>,
    ...
    pub analysis_profiles: RwLock<HashMap<String, cortex_core::graph::AnalysisProfile>>,

    // Topology rules (keyed by rule id String)
    pub topology_rules: RwLock<HashMap<String, cortex_core::graph::TopologyRule>>,
...
impl GraphStore for MockGraphStore {
    ...
    async fn delete_file(&self, path: &str) -> Result<()> {
        ...
        // Remove symbols linked to this file (mirrors DETACH DELETE in IndentiaGraph)
    ...
    async fn batch_create_call_relationships(
        &self,
        calls: &[cortex_core::parser_types::FunctionCall],
        project_id: Option<Uuid>,
    ) -> Result<()> {
        for call in calls {
    ...
    async fn cleanup_builtin_calls(&self) -> Result<i64> {
        // Minimal builtin set for mock (full noise_filter lives in cortex-parser)
        const BUILTINS: &[&str] = &[
            "println",
            "eprintln",
            "print",
        ...
        let builtin_set: std::collections::HashSet<&str> = BUILTINS.iter().copied().collect();
        let mut cr = self.call_relationships.write().await;
        let mut deleted = 0i64;
        for (_caller_id, callees) in cr.iter_mut() {
            let before = callees.len();
            callees.retain(|callee_name| !builtin_set.contains(callee_name.as_str()));
    ...
    async fn get_code_health_report(
        ...
        // --- Coupling metrics ---
        let fa = self.file_analytics.read().await;
        let project_analytics: Vec<&cortex_core::graph::FileAnalyticsUpdate> = fa
            .values()
    ...
    async fn search_decisions_by_vector(
        ...
        // Mock: return empty — vector search requires real IndentiaGraph index
        Ok(vec![])
    }
        ...
        if let Some(workspace) = ws {
            ...
            Ok(ws_notes
                ...
                .map(|n| PropagatedNote {
                    ...
                    note: n,
                    path_pagerank: None,
                    relation_path: vec![cortex_core::notes::RelationHop::structural("BELONGS_TO")],
    ...
    async fn update_energy_scores(&self, half_life_days: f64) -> Result<usize> {
        ...
        for note in notes.values_mut() {
            if note.status != cortex_core::notes::NoteStatus::Active || note.energy <= 0.0 {
                continue;
            }
    ...
    async fn list_notes_needing_synapses(
        ...
        let needing: Vec<cortex_core::notes::Note> = notes
            .values()
        ...
        let batch: Vec<cortex_core::notes::Note> = needing.into_iter().take(limit).collect();
        Ok((batch, total))
    }
    ...
    async fn upsert_user(&self, user: &UserNode) -> Result<UserNode> {
        use cortex_core::models::AuthProvider;
    ...
    async fn get_user_by_provider_id(
        ...
        let provider_parsed: cortex_core::models::AuthProvider =
    ...
    async fn create_password_user(
        ...
        let user = UserNode {
            ...
            name: name.to_string(),
            picture_url: None,
            auth_provider: cortex_core::models::AuthProvider::Password,
            external_id: None,
    ...
    async fn create_refresh_token(
        ...
        let token = cortex_core::models::RefreshTokenNode {
    ...
    async fn get_project_call_edges(
        ...
        for (caller_id, callees) in cr.iter() {
            ...
            // Build function id matching IndentiaGraph format: "file_path:name:line_start"
    ...
    async fn batch_update_file_analytics(
        &self,
        updates: &[cortex_core::graph::FileAnalyticsUpdate],
    ) -> anyhow::Result<()> {
    ...
    async fn batch_update_function_analytics(
        &self,
        updates: &[cortex_core::graph::FunctionAnalyticsUpdate],
    ) -> anyhow::Result<()> {
    ...
    async fn batch_update_fabric_file_analytics(
        &self,
        _updates: &[cortex_core::graph::FabricFileAnalyticsUpdate],
    ) -> anyhow::Result<()> {
    ...
    async fn batch_update_structural_dna(
        &self,
        _updates: &[cortex_core::graph::StructuralDnaUpdate],
    ) -> anyhow::Result<()> {
    ...
    async fn batch_update_structural_fingerprints(
        &self,
        _updates: &[cortex_core::graph::StructuralFingerprintUpdate],
    ) -> anyhow::Result<()> {
    ...
    async fn get_project_file_signals(
        &self,
        _project_id: &str,
    ) -> anyhow::Result<Vec<cortex_core::graph::FileSignalRecord>> {
    ...
    async fn write_predicted_links(
        &self,
        _project_id: &str,
        _links: &[cortex_core::graph::LinkPrediction],
    ) -> anyhow::Result<()> {
    ...
    async fn get_neural_metrics(
        &self,
        _project_id: Uuid,
    ) -> anyhow::Result<cortex_core::models::NeuralMetrics> {
        Ok(cortex_core::models::NeuralMetrics {
            active_synapses: 0,
            avg_energy: 0.0,
    ...
    async fn compute_churn_scores(
        &self,
        _project_id: Uuid,
    ) -> anyhow::Result<Vec<cortex_core::models::FileChurnScore>> {
        Ok(vec![])
    }

    async fn batch_update_churn_scores(
        &self,
        _updates: &[cortex_core::models::FileChurnScore],
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    async fn compute_knowledge_density(
        &self,
        _project_id: Uuid,
    ) -> anyhow::Result<Vec<cortex_core::models::FileKnowledgeDensity>> {
        Ok(vec![])
    }

    async fn batch_update_knowledge_density(
        &self,
        _updates: &[cortex_core::models::FileKnowledgeDensity],
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    async fn compute_risk_scores(
        &self,
        _project_id: Uuid,
    ) -> anyhow::Result<Vec<cortex_core::models::FileRiskScore>> {
        Ok(vec![])
    }

    async fn batch_update_risk_scores(
        &self,
        _updates: &[cortex_core::models::FileRiskScore],
    ) -> anyhow::Result<()> {
        Ok(())
    }
    ...
    async fn create_topology_rule(
        &self,
        rule: &cortex_core::graph::TopologyRule,
    ) -> anyhow::Result<()> {
    ...
    async fn list_topology_rules(
        &self,
        project_id: &str,
    ) -> anyhow::Result<Vec<cortex_core::graph::TopologyRule>> {
    ...
    async fn check_topology_rules(
        &self,
        _project_id: &str,
    ) -> anyhow::Result<Vec<cortex_core::graph::TopologyViolation>> {
    ...
    async fn create_analysis_profile(
        &self,
        profile: &cortex_core::graph::AnalysisProfile,
    ) -> anyhow::Result<()> {
    ...
    ) -> anyhow::Result<Vec<cortex_core::graph::AnalysisProfile>> {
    ...
    async fn get_analysis_profile(
        &self,
        id: &str,
    ) -> anyhow::Result<Option<cortex_core::graph::AnalysisProfile>> {
        let profiles = self.analysis_profiles.read().await;
        ...
        let count = notes
            ...
            .filter(|n| {
                n.anchors.iter().any(|a| {
                    a.entity_type == cortex_core::notes::EntityType::File
                        && a.entity_id == file_path
                })
            })
            .count();
    ...
    async fn find_bridge_subgraph(
        ...
        use cortex_core::graph::{BridgeRawEdge, BridgeRawNode};
    ...
    async fn create_skill(&self, skill: &cortex_core::skills::SkillNode) -> anyhow::Result<()> {
    ...
    async fn get_skill(&self, id: Uuid) -> anyhow::Result<Option<cortex_core::skills::SkillNode>> {
        Ok(self.skills.read().await.get(&id).cloned())
    }

    async fn update_skill(&self, skill: &cortex_core::skills::SkillNode) -> anyhow::Result<()> {
    ...
    async fn list_skills(
        &self,
        project_id: Uuid,
        status: Option<cortex_core::skills::SkillStatus>,
        limit: usize,
        offset: usize,
    ) -> anyhow::Result<(Vec<cortex_core::skills::SkillNode>, usize)> {
        ...
        let mut filtered: Vec<_> = store
            ...
            .cloned()
            .collect();
        // Sort by energy DESC, name ASC (consistent with IndentiaGraph query)
    ...
    async fn get_skill_members(
        &self,
        skill_id: Uuid,
    ) -> anyhow::Result<(Vec<cortex_core::notes::Note>, Vec<DecisionNode>)> {
    ...
    async fn add_skill_member(
        ...
        if !list.contains(&entry) {
            list.push(entry);
            // Update counters on the skill (consistent with IndentiaGraph behavior)
            drop(members);
    ...
    async fn get_skills_for_note(
        &self,
        note_id: Uuid,
    ) -> anyhow::Result<Vec<cortex_core::skills::SkillNode>> {
        let members = self.skill_members.read().await;
        ...
        // Sort by energy DESC for consistency with IndentiaGraph query
        result.sort_by(|a, b| {
            b.energy
                .partial_cmp(&a.energy)
                .unwrap_or(std::cmp::Ordering::Equal)
    ...
    async fn activate_skill(
        ...
        skill_id: Uuid,
        _query: &str,
    ) -> anyhow::Result<cortex_core::skills::ActivatedSkillContext> {
        let skill = self
            .get_skill(skill_id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("Skill not found: {}", skill_id))?;
        // Return actual member notes/decisions (consistent with IndentiaGraph behavior)
        let (member_notes, relevant_decisions) = self.get_skill_members(skill_id).await?;
        // Wrap notes as ActivatedNote
        let activated_notes: Vec<cortex_core::neurons::ActivatedNote> = member_notes
            .iter()
            .map(|n| cortex_core::neurons::ActivatedNote {
                note: n.clone(),
        ...
        Ok(cortex_core::skills::ActivatedSkillContext {
            skill,
            activated_notes,
            relevant_decisions,
            context_text,
    ...
    async fn match_skills_by_trigger(
        ...
        project_id: Uuid,
        input: &str,
    ) -> anyhow::Result<Vec<(cortex_core::skills::SkillNode, f64)>> {
        let store = self.skills.read().await;
        let candidates: Vec<_> = store
            ...
            .filter(|s| {
                s.project_id == project_id
                    && matches!(
                        s.status,
                        cortex_core::skills::SkillStatus::Active
                            | cortex_core::skills::SkillStatus::Emerging
                    )
            })
            .cloned()
            .collect();
        ...
        for skill in candidates {
            ...
            for trigger in &skill.trigger_patterns {
                let matched = match trigger.pattern_type {
                    cortex_core::skills::TriggerType::Regex => {
                        regex::RegexBuilder::new(&format!("(?i){}", &trigger.pattern_value))
                            .build()
                            .map(|re| re.is_match(input))
                            .unwrap_or(false)
                    }
                    cortex_core::skills::TriggerType::FileGlob => {
                        glob::Pattern::new(&trigger.pattern_value)
                            .map(|g| g.matches(input))
                            .unwrap_or(false)
                    }
                    cortex_core::skills::TriggerType::Semantic => {
                        // Semantic triggers can't be evaluated in mock
                        false
                    }
                    cortex_core::skills::TriggerType::McpAction => {
                        // Simple prefix match for MCP action triggers in mock
                        input.starts_with(&trigger.pattern_value) || trigger.pattern_value == input
                    }
                };
                if matched {
            ...
            let final_confidence = if skill.status == cortex_core::skills::SkillStatus::Emerging {
                confidence * 0.8
            } else {
                confidence
            };
        ...
        // Sort by energy DESC for consistency with IndentiaGraph
        results.sort_by(|a, b| {
            b.0.energy
    ...
    async fn batch_save_context_cards(
        &self,
        _cards: &[cortex_core::graph::ContextCard],
    ) -> anyhow::Result<()> {
    ...
    async fn get_context_card(
        ...
        _path: &str,
        _project_id: &str,
    ) -> anyhow::Result<Option<cortex_core::graph::ContextCard>> {
    ...
    async fn get_context_cards_batch(
        ...
        _paths: &[String],
        _project_id: &str,
    ) -> anyhow::Result<Vec<cortex_core::graph::ContextCard>> {
        // Mock: no context cards stored
        Ok(Vec::new())
    }
    ...
    ) -> anyhow::Result<Vec<cortex_core::graph::IsomorphicGroup>> {
    ...
    async fn search_notes_fts(
        ...
        limit: usize,
        project_id: Option<&str>,
    ) -> anyhow::Result<Vec<(cortex_core::notes::Note, f64)>> {
        // Mock: simple case-insensitive keyword match on content.
        let kw = query.to_lowercase();
        let notes = self.notes.read().await;
        let mut results: Vec<(cortex_core::notes::Note, f64)> = notes
            .values()
            .filter(|n| {
                let content_match = n.content.to_lowercase().contains(&kw);
                let tag_match = n.tags.iter().any(|t| t.to_lowercase().contains(&kw));
                let project_match = match project_id {
                    Some(pid) => n
                        .project_id
                        .as_ref()
                        .map(|id| id.to_string() == pid)
                        .unwrap_or(false),
                    None => true,
                };
                (content_match || tag_match) && project_match
            })
            .map(|n| (n.clone(), 1.0f64))
            .collect();
    ...
    async fn search_decisions_fts(
        ...
        limit: usize,
        _project_id: Option<&str>,
    ) -> anyhow::Result<Vec<(cortex_core::models::DecisionNode, f64)>> {
        // Mock: simple case-insensitive keyword match on description + rationale.
        let kw = query.to_lowercase();
        let decisions = self.decisions.read().await;
        let mut results: Vec<(cortex_core::models::DecisionNode, f64)> = decisions
            .values()
            .filter(|d| {
                d.description.to_lowercase().contains(&kw)
                    || d.rationale.to_lowercase().contains(&kw)
            })
            .map(|d| (d.clone(), 1.0f64))
            .collect();
        results.truncate(limit);
        Ok(results)
    }
    ...
    async fn search_code_fts(
        ...
        project_id: Option<&str>,
        language: Option<&str>,
    ) -> anyhow::Result<Vec<crate::CodeSearchHit>> {
        // Mock: simple case-insensitive keyword match on function/struct names.
        let kw = query.to_lowercase();
        let functions = self.functions.read().await;
        let structs = self.structs_map.read().await;

        let mut by_path: std::collections::HashMap<String, crate::CodeSearchHit> =
            std::collections::HashMap::new();

        for func in functions.values() {
            let matches = func.name.to_lowercase().contains(&kw)
                || func
                    .docstring
                    .as_deref()
                    .unwrap_or("")
            ...
            if matches {
                ...
                let entry =
                    by_path
                        .entry(func.file_path.clone())
                        .or_insert_with(|| crate::CodeSearchHit {
                            path: func.file_path.clone(),
                            language: lang.clone(),
                            symbols: Vec::new(),
                            docstring: func.docstring.clone().unwrap_or_default(),
                            score: 1.0,
                            project_id: project_id.map(|s| s.to_string()),
                            project_slug: None,
                        });
                if !entry.symbols.contains(&func.name) {
                    entry.symbols.push(func.name.clone());
                }
            }
        }

        for s in structs.values() {
            let matches = s.name.to_lowercase().contains(&kw)
                || s.docstring
                    .as_deref()
                    .unwrap_or("")
                    .to_lowercase()
            ...
            if matches {
                ...
                if !entry.symbols.contains(&s.name) {
                    entry.symbols.push(s.name.clone());
                }
            }
        }

        let mut hits: Vec<crate::CodeSearchHit> = by_path.into_values().collect();
    ...
    async fn add_episode(
        ...
        Ok(cortex_core::episode::Episode {
            id: format!("episode:{}", Uuid::new_v4()),
            name: req.name,
            content: req.content,
            source: req.source,
            reference_time: req.reference_time.unwrap_or_else(Utc::now),
            ingested_at: Utc::now(),
    ...
    async fn invalidate_note_at(&self, _id: &str, _at: DateTime<Utc>) -> Result<()> {
        Ok(())
    }
...
mod tests {
    ...
    async fn test_community_filter_excludes_transitive_different_community() {
        ...
        use cortex_core::graph::FunctionAnalyticsUpdate;
        store
    ...
    async fn test_auto_build_with_vector_expansion() {
        ...
        // Set same community for entry_fn and unrelated_fn (community filtering enabled)
        {
            ...
            fa.insert(
                "entry_fn".to_string(),
                cortex_core::graph::FunctionAnalyticsUpdate {
    ...
    async fn test_auto_build_with_community_expansion() {
        ...
        // Set file analytics: routes.rs and handlers.rs in community 1, client.rs in community 2
        {
            ...
            fa.insert(
                ...
                cortex_core::graph::FileAnalyticsUpdate {
    ...
    async fn test_batch_create_call_relationships() {
        let store = MockGraphStore::new();
        let calls = vec![
            cortex_core::parser_types::FunctionCall {
    ...
    async fn test_count_project_plans_filters_by_status() {
        ...
        let mut plan_draft = cortex_core::test_helpers::test_plan_for_project(project.id);
        ...
        let mut plan_in_progress = cortex_core::test_helpers::test_plan_for_project(project.id);
        ...
        let mut plan_completed = cortex_core::test_helpers::test_plan_for_project(project.id);
