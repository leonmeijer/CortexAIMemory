//! IndentiaGraph Knowledge Note operations
...
impl IndentiaGraphClient {
    ...
    // Helper function to convert IndentiaGraph node to Note
    ...
    // ========================================================================
    // Bi-temporal operations (Graphiti-inspired)
    // ========================================================================

    /// Mark a note as invalid at a specific point in time (bi-temporal).
    /// Sets `invalid_at` and status to 'obsolete'. The note is preserved for history.
    pub async fn invalidate_note_at(
        &self,
        id: Uuid,
        ...
        let q = query(
            r#"
            MATCH (n:Note {id: $id})
            SET n.invalid_at = $at, n.status = 'obsolete'
            "#,
        )
        .param("id", id.to_string())
    ...
    pub async fn search_notes_at_time(
        ...
        let (cypher_str, q) = if let Some(pid) = project_id {
            let q = query(
                r#"
                MATCH (n:Note {project_id: $project_id})
                WHERE toLower(n.content) CONTAINS toLower($query)
                  AND (n.valid_at IS NULL OR n.valid_at <= $at)
                  AND (n.invalid_at IS NULL OR n.invalid_at > $at)
        ...
        let _ = cypher_str; // suppress unused warning
        let mut result = self.graph.execute(q).await?;
        let mut notes = Vec::new();
        while let Some(row) = result.next().await? {
    ...
    /// Full-text keyword search across notes using Cypher toLower + CONTAINS.
    pub async fn search_notes_fts(
        &self,
        query: &str,
        _filters: &crate::notes::NoteFilters,
    ) -> anyhow::Result<Vec<crate::notes::NoteSearchHit>> {
        use crate::notes::NoteSearchHit;
        let cypher = r#"
            MATCH (n:Note)
            WHERE (toLower(n.content) CONTAINS toLower($q)
                   OR any(tag IN n.tags WHERE toLower(tag) CONTAINS toLower($q)))
              AND NOT n.status IN ['obsolete', 'archived']
            RETURN n
            LIMIT 50
        "#;
        let mut result = self
            .graph
            .execute(neo4rs::query(cypher).param("q", query))
            .await?;
