//! IndentiaGraph Code structure, symbols, imports, calls, exploration, impact, and embeddings

use super::client::IndentiaGraphClient;
use super::models::*;
use anyhow::Result;
use neo4rs::query;
use uuid::Uuid;
...
impl IndentiaGraphClient {
    ...
    /// can also clean secondary search indexes.
    ...
    /// Batch upsert functions using UNWIND for a single IndentiaGraph transaction per call.
    ...
    ///
    /// Both phases are chunked (BATCH_SIZE items per query) to avoid IndentiaGraph OOM/timeout
    ...
    ///
    /// Chunked (BATCH_SIZE items per query) to avoid IndentiaGraph OOM/timeout.
    pub async fn batch_create_implements_relationships(
        ...
        use crate::indentiagraph::batch::run_unwind_in_chunks;
    ...
    /// Clean up ALL sync-generated data from IndentiaGraph.
    ...
    pub async fn batch_upsert_processes(
        &self,
        processes: &[crate::indentiagraph::models::ProcessNode],
    ) -> Result<()> {
    ...
    // ========================================================================
    // Full-text search (BM25 via Cypher CONTAINS)
    // ========================================================================

    /// Full-text keyword search across code symbols using Cypher toLower + CONTAINS.
    /// Searches function names (and docstrings) as well as file paths.
    pub async fn search_code_fts(
        ...
        language: Option<&str>,
    ) -> anyhow::Result<Vec<CodeSearchHit>> {
        // Build conditional WHERE clauses for optional filters
        let lang_filter = if language.is_some() {
            "AND file.language = $lang"
        } else {
            ""
        };
        let proj_filter = if project_id.is_some() {
            "AND file.project_id = $project_id"
        } else {
            ""
        };

        // Query 1: functions whose name/docstring matches, grouped by file
        let cypher_funcs = format!(
            r#"
            MATCH (func:Function)
            WHERE toLower(func.name) CONTAINS toLower($q)
               OR (func.docstring IS NOT NULL AND toLower(func.docstring) CONTAINS toLower($q))
            MATCH (file:File {{path: func.file_path}})
            WHERE 1=1 {lang} {proj}
            WITH file, collect(func.name) AS symbols
            RETURN file.path AS path, file.language AS lang,
                   coalesce(file.project_id, '') AS project_id, symbols
            LIMIT $limit
            "#,
            lang = lang_filter,
            proj = proj_filter,
        ...
        // Query 2: files whose path matches
        let cypher_files = format!(
            r#"
            MATCH (file:File)
            WHERE toLower(file.path) CONTAINS toLower($q)
              {lang} {proj}
            RETURN file.path AS path, file.language AS lang,
                   coalesce(file.project_id, '') AS project_id, [] AS symbols
            LIMIT $limit
            "#,
            lang = lang_filter,
            proj = proj_filter,
        ...
        let mut hits: std::collections::HashMap<String, CodeSearchHit> =
            std::collections::HashMap::new();

        // Build base query with required params, then add optional params
        let build_q = |cypher: &str| {
            let mut q = neo4rs::query(cypher)
                .param("q", query.to_string())
                .param("limit", limit as i64);
            if let Some(lang) = language {
                q = q.param("lang", lang.to_string());
            }
        ...
        // Run function name search
        let mut result = self.graph.execute(build_q(&cypher_funcs)).await?;
        while let Some(row) = result.next().await? {
            let path: String = row.get("path").unwrap_or_default();
            let lang: String = row.get("lang").unwrap_or_default();
            let symbols: Vec<String> = row.get("symbols").unwrap_or_default();
            let pid_str: String = row.get("project_id").unwrap_or_default();
            let pid_uuid = pid_str.parse::<uuid::Uuid>().ok();
            let entry = hits.entry(path.clone()).or_insert(CodeSearchHit {
                path,
                language: lang,
                symbols: vec![],
        ...
        // Run file path search
        let mut result2 = self.graph.execute(build_q(&cypher_files)).await?;
        while let Some(row) = result2.next().await? {
            let path: String = row.get("path").unwrap_or_default();
            let lang: String = row.get("lang").unwrap_or_default();
            let pid_str: String = row.get("project_id").unwrap_or_default();
            let pid_uuid = pid_str.parse::<uuid::Uuid>().ok();
            hits.entry(path.clone()).or_insert(CodeSearchHit {
                path,
                language: lang,
                symbols: vec![],
                docstring: String::new(),
        ...
        let mut results: Vec<CodeSearchHit> = hits.into_values().collect();
