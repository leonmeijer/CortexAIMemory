//! IndentiaGraph Skill operations
//!
//! Implements all CRUD, membership, query, and activation operations
//! for the Neural Skills system on the IndentiaGraph graph.
...
impl IndentiaGraphClient {
    ...
    /// Convert a IndentiaGraph node to a [`SkillNode`].
    ...
    pub async fn get_skill_members(
        ...
        while let Some(row) = notes_result.next().await? {
            ...
            // IndentiaGraph returns collect() as a BoltList — neo4rs deserializes it
...
mod tests {
    ...
    fn test_node_to_skill_trigger_serialization() {
        ...
        let triggers = vec![
            SkillTrigger::regex("indentiagraph|cypher", 0.7),
    ...
    fn test_trigger_regex_matching() {
        let re = regex::Regex::new("indentiagraph|cypher|UNWIND").unwrap();
        assert!(re.is_match("UNWIND $items AS item"));
        assert!(re.is_match("indentiagraph client"));
        assert!(!re.is_match("python django"));
    }

    #[test]
    fn test_trigger_glob_matching() {
        let pattern = glob::Pattern::new("src/indentiagraph/**").unwrap();
        assert!(pattern.matches("src/indentiagraph/skill.rs"));
        assert!(pattern.matches("src/indentiagraph/client.rs"));
