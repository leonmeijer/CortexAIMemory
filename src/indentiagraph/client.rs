//! IndentiaGraph client for interacting with the knowledge graph

use anyhow::{Context, Result};
use neo4rs::{query, Graph, Query};
use std::sync::Arc;

/// Client for IndentiaGraph operations
pub struct IndentiaGraphClient {
...
impl WhereBuilder {
    ...
    /// Add a status filter (converts snake_case to PascalCase for IndentiaGraph)
...
impl IndentiaGraphClient {
    /// Create a new IndentiaGraph client
    pub async fn new(uri: &str, user: &str, password: &str) -> Result<Self> {
        let graph = Graph::new(uri, user, password)
            .await
            .context("Failed to connect to IndentiaGraph")?;
    ...
    /// Log a warning if a IndentiaGraph query exceeded the slow query threshold.
    ///
    /// Usage:
    /// ```rust,ignore
    ...
    /// ```
    ///
    /// Threshold is configurable via env `INDENTIAGRAPH_SLOW_QUERY_THRESHOLD_MS` (default 100ms).
    pub fn log_slow_query(&self, label: &str, start: std::time::Instant) {
        let elapsed_ms = start.elapsed().as_millis() as u64;

        let threshold: u64 = std::env::var("INDENTIAGRAPH_SLOW_QUERY_THRESHOLD_MS")
            .ok()
        ...
        if elapsed_ms > threshold {
            tracing::warn!(
                ...
                label,
                "Slow IndentiaGraph query detected"
            );
        } else {
            tracing::trace!(query_ms = elapsed_ms, label, "IndentiaGraph query completed");
        }
    }
    ...
    async fn init_schema(&self) -> Result<()> {
        ...
        // Vector indexes (require IndentiaGraph 5.13+ — gracefully skip if not supported)
        ...
        // ---------------------------------------------------------------
        // Skill relations (not enforced by IndentiaGraph, documented here):
        ...
        // Vector indexes — optional, don't fail startup if IndentiaGraph doesn't support them
        for vi in vector_indexes {
            if let Err(e) = self.graph.run(query(vi)).await {
                tracing::warn!(
                    "Vector index creation skipped (IndentiaGraph may not support vector indexes): {}",
                    e
                );
            }
        }
