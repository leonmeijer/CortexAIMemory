//! IndentiaGraph Feature Graph operations
...
impl IndentiaGraphClient {
    ...
    ///
    /// For each input function that has an embedding stored in IndentiaGraph, performs HNSW vector
    ...
    ///
    /// For each input file that has a `community_id` in IndentiaGraph, finds other files in the
