//! IndentiaGraph Chat Session and Event operations
...
impl IndentiaGraphClient {
    ...
    pub async fn get_session_auto_continue(&self, id: Uuid) -> Result<bool> {
        ...
        let mut result = self.graph.execute(q).await?;
        if let Some(row) = result.next().await? {
            // IndentiaGraph may return null if property not set
            Ok(row
    ...
    /// Uses the first user_message event stored in IndentiaGraph.
    ...
    /// Parse a IndentiaGraph Node into a ChatSessionNode
    ...
    /// Parse a IndentiaGraph Node into a ChatEventRecord
