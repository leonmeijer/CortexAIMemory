//! IndentiaGraph Decision operations
...
impl IndentiaGraphClient {
    ...
    /// Used by the reindex command to rebuild decision search data.
    ...
    /// Full-text keyword search across decisions using Cypher toLower + CONTAINS.
    pub async fn search_decisions_fts(
        ...
        let cypher = if project_id.is_some() {
            r#"
            MATCH (t:Task)-[:INFORMED_BY]->(d:Decision)
            MATCH (p:Plan {id: t.plan_id})
            OPTIONAL MATCH (p)-[:BELONGS_TO]->(proj:Project)
            WHERE (toLower(d.description) CONTAINS toLower($q)
               OR toLower(d.rationale) CONTAINS toLower($q))
              AND (p.project_id = $project_id OR proj.id = $project_id)
            RETURN DISTINCT d
            ORDER BY d.decided_at DESC
            LIMIT $limit
        "#
        ...
        if let Some(project_id) = project_id {
            q = q.param("project_id", project_id.to_string());
        }

        let mut result = self.graph.execute(q).await?;
        let mut hits = Vec::new();
        while let Some(row) = result.next().await? {
            if let Ok(node) = row.get::<neo4rs::Node>("d") {
                if let Ok(decision) = Self::node_to_decision(&node) {
                    hits.push((decision, 1.0f64));
                }
            }
        }
        Ok(hits)
